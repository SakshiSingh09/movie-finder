...
export default function Drama(){
    ...
    async function getDramaMovies(){
        const url = "https://api.themoviedb.org/3/discover/movie?api_key=45c19e5ad1a4792ab884744bb269265b&with_genres=18";
        const responsedata = await fetch(url).then(response => response.json());
        setDramaMovie(responsedata.results)
      }
    
    useEffect(()=>{
        getDramaMovies();
        document.title = 'Drama Movies';
    },[])
    ...
    if (dramaMovies.length !== 0){
        return(
            <div>
                <h1 className="right-heading">Drama Movies</h1>           
