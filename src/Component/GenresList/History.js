...
export default function History(){
    ...
    async function gethistoryMovies(){
        const url = "https://api.themoviedb.org/3/discover/movie?api_key=45c19e5ad1a4792ab884744bb269265b&with_genres=36";
        const responsedata = await fetch(url).then(response => response.json());
        sethistoryMovies(responsedata.results);
      }
    
    useEffect(()=>{
        gethistoryMovies();
        document.title = 'History Movies';
    },[])
    ...
    if (historyMovies.length !== 0){
        return(
            <div>
                <h1 className="right-heading">History Movies</h1>           
