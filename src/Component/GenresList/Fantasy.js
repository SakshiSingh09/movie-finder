...
export default function Fantasy(){
    ...
    async function getfantasyMovies(){
        const url = "https://api.themoviedb.org/3/discover/movie?api_key=45c19e5ad1a4792ab884744bb269265b&with_genres=14";
        const responsedata = await fetch(url).then(response => response.json());
        setfantasyMovies(responsedata.results);
      }
    
    useEffect(()=>{
        getfantasyMovies();
        document.title = 'Fantasy Movies';
    },[])
    ...
    if (fantasyMovies.length !== 0){
        return(
            <div>
                <h1 className="right-heading">Fantasy Movies</h1>           
