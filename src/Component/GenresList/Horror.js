...
export default function Horror(){
    ...
    async function gethorrorMovies(){
        const url = "https://api.themoviedb.org/3/discover/movie?api_key=45c19e5ad1a4792ab884744bb269265b&with_genres=27";
        const responsedata = await fetch(url).then(response => response.json());
        sethorrorMovies(responsedata.results);
      }
    
    useEffect(()=>{
        gethorrorMovies();
        document.title = 'Horror Movies';
    },[])
    ...
    if (horrorMovies.length !== 0){
        return(
            <div>
                <h1 className="right-heading">Horror Movies</h1>           
